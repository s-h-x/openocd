...
/**
 * The value of @c address determines if it corresponds to P: (program) or X: (dat) memory.
 * If the address is over 0x200000 then it is considered X: memory, and @c pmem = 0.
 ...
 *
 * @return ERROR_OK
 * @bug trivial return value (always ERROR_OK)
 */
...
/**
 * Writes @c buffer to memory.
 * The parameter @c address determines whether @c buffer should be written to
 * P: (program) memory or X: (dat) memory.
 *
 * @param t target
 * @param a address
 * @param s size Bytes (1), Half words (2), Words (4).
 * @param c count In bytes.
 * @param b buffer
 *
 * @return error status
 */
 ...
 *
 * @param t target
 * @param a address Word addressing.
 * @param size In bytes.
 * @param buf
 *
 * @return error status
 ...
 *
 * @param t target
 * @param a address Start address of the image.
...
/**
 * Calculates a signature over @c word_count words in the data from @c buff8.
 ...
 * Some commands use the parameters @c address and @c data, others ignore them.
 *
 * @param t target
 * @param c command Command to execute.
 * @param a address Command parameter.
 * @param d data Command parameter.
 * @param h hfm_ustat FM status register.
 * @param p pmem Address is P: (program) memory (@c pmem == 1) or X: (dat) memory (@c pmem == 0)
 *
 * @return error status
 */
...
/**
 * Executes the FM calculate signature command. The FM will calculate over the data from @c address to @c address + @c words -1. The result is written to a register, then read out by this function and returned in @c signature. The value @c signature may be compared to the the one returned by perl_crc to verify the flash was written correctly.
 *
 * @param t target
 * @param a address Start of flash array where the signature should be calculated.
